...
export default function ContactCTA({ productId, productTitle, size = 'md' }) {
  ...
  // Uniform CTA classes for both sizes: uppercase, large padding, no border radius, simple hover
  const baseClass = "flex-1 flex items-center justify-center gap-3 bg-[#3665f3] hover:bg-[#2b51c2] text-white font-bold uppercase tracking-wider transition-colors rounded-none w-full cursor-pointer";
  const paddingClass = size === 'lg' ? "py-5 px-6 text-base" : "py-4 px-4 text-sm";

  return (
    <div className={`flex flex-col sm:flex-row gap-px bg-gray-200 ${size === 'lg' ? 'mt-4' : ''}`}>
      <button
